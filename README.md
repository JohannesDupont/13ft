# Docker instruction: 
bash script. Simply run ` bash run.sh ` to build and run the container. 
port is opened to ` 0.0.0.0:500 ` - change port config in python file
the container shuts down automatically when ctrl c
